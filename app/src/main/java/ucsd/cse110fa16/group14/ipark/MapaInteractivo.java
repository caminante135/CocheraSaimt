...
*  MapaInteractivo mp;
*  mp = new MapaInteractivo(this);
*  setContentView(mp);*/

// Map for owner
public class MapaInteractivo extends View {
    ...
    public MapaInteractivo(Context context) {
        super(context);
        init(context);
    }

    public MapaInteractivo(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public MapaInteractivo(Context context, AttributeSet attrs, int defStyle) {
    ...
    Runnable longPressed = new Runnable() {
        public void run() {
            ...
            long curTimeInSec = iLink.getCurTimeInSec();
            parkingLotStatus = iLink.getParkingLotStatus(curTimeInSec, curTimeInSec);
            builder.setTitle("Cambiar el estado del sitio");

            int myInx = parkingLotStatus[rectIndex];

            if (myInx == iLink.AVAILABLE) {
                builder.setMessage("    ¿Cambiar el lugar dispobible a reservado?\n");
            } else if (myInx == iLink.OWNER_RESERVED) {
                builder.setMessage("    ¿Cambiar el lugar disponible a reservado?\n");
            } else if (myInx == iLink.ILLEGAL) {
                builder.setMessage("    Cambiar el lugar a disponible?\n");
            } else builder.setMessage("    No se puede cambiar ocupado.\n");
